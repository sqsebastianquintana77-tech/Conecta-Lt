    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="relative h-64 md:h-80 bg-gradient-to-br from-primary to-primary/80">
        ...
        <div className="relative z-10 h-full flex flex-col justify-end p-6 max-w-4xl mx-auto w-full">
          <Link href="/" className="inline-flex items-center text-white/70 hover:text-white mb-4 text-sm transition-colors duration-200">
          ...
          <div className="flex items-center gap-3 text-white/90 text-sm">
            ...
            <span className="flex items-center gap-1"><Star size={14} className="text-primary fill-primary" /> {business.rating} ({business.reviewCount})</span>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
            <Card>
              <CardContent className="p-6">
                <h2 className="text-lg font-semibold text-foreground mb-3">Sobre {business.name}</h2>
                <p className="text-foreground/80 leading-relaxed">{business.description}</p>
                {business.specialty && (
                  <p className="mt-3 text-sm"><span className="font-medium text-foreground">Especialidad:</span> <span className="text-primary">{business.specialty}</span></p>
                )}
                {business.topBrands && (
                  <p className="mt-1 text-sm"><span className="font-medium text-foreground">Marcas destacadas:</span> <span className="text-muted-foreground">{business.topBrands}</span></p>
                )}
              </CardContent>
            </Card>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
            ...
            <div className="grid grid-cols-3 gap-2">
              {business.gallery.slice(0, 3).map((img, i) => (
                <button key={i} onClick={() => { setGalleryIdx(i); setGalleryOpen(true); }} className="relative aspect-square rounded-2xl overflow-hidden group cursor-pointer">
                  <img src={img} alt={`${business.name} foto ${i + 1}`} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300 shadow-soft hover:shadow-card-hover" />
            ...
            {galleryOpen && (
              <div className="fixed inset-0 z-[200] bg-black/90 flex items-center justify-center p-4">
                ...
                <img src={business.gallery[galleryIdx]} alt={`${business.name} foto ${galleryIdx + 1}`} className="max-w-full max-h-[80vh] object-contain rounded-xl" />
              ...
              {(business as unknown as { promotions: Promotion[] }).promotions.map((promo) => (
                <Card key={promo.id} className="border-red-500/20 bg-red-500/10">
                  <CardContent className="p-4 flex items-start justify-between">
                    ...
                    {promo.discount && <Badge className="bg-red-500 text-white border-0 shrink-0">{promo.discount}</Badge>}
                  </CardContent>
                </Card>
              ))}
            </div>
        ...
        <div className="flex flex-wrap gap-3 pb-8">
          {business.whatsapp && (
            <a href={`https://wa.me/${business.whatsapp}`} target="_blank" rel="noopener noreferrer">
              <Button className="bg-green-600 hover:bg-green-700 text-white gap-2 transition-colors duration-200"><MessageCircle size={18} /> WhatsApp</Button>
            </a>
          )}
          {business.instagram && (
            <a href={`https://instagram.com/${business.instagram.replace('@', '')}`} target="_blank" rel="noopener noreferrer">
              <Button variant="outline" className="gap-2 transition-colors duration-200"><ExternalLink size={18} /> Instagram</Button>
            </a>
          )}
          {business.website && (
